proc highlightingWorstIDpaths {args} {
    ...
    foreach sink $worstSinks {
        ...
        if { [lsort -unique [get_db selected .obj_type]] == "pin" } { 
            ...
            report_timing -unconst -path_type full_clock  -to [get_db pin:$sink .inst.name] -from [get_db pin:[lindex [get_skew_group_path -skew_group $sg -sink $sink] 0] .inst.name]
            if { $i == 32 } {
                set i 0
            }
        } else {
            gui_ctd_highlight -to [get_db pin:$sink .inst.name] -from [lindex [get_skew_group_path -skew_group $sg -sink $sink] 0] -index $i
            report_timing  -unconst -path_type full_clock   -to [get_db pin:$sink .inst.name] -from [lindex [get_skew_group_path -skew_group $sg -sink $sink] 0] 
            if { $i == 32 } {
                set i 0
            }
        }
