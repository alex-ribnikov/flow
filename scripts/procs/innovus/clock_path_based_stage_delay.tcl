...
#    puts "$root NA $root_delay $root_delay"
    set table [list [list $root NA $root_delay $root_delay]]
...
#            puts "list $i [get_db pin:$i .inst.base_cell.base_name] [get_skew_group_delay -skew_group $skew_group -to $i] [expr $delay - $prev_delay]"
            lappend table [list $i [get_db pin:$i .inst.base_cell.base_name] [get_skew_group_delay -skew_group $skew_group -to $i] [expr $delay - $prev_delay]]
            set prev_delay [expr $delay + $root_delay]
        } 
    }
    
    set header [list "ClockPort/Pin" Net CellType ArrivalTime Delay WireLength]
    rls_table -table $table -breaks -spac -header $header
