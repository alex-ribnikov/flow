...
set M1_offset               [expr (17*$M1_pitch) + 0.0085 -$M1_width/2]
set M1_spacing              "[expr (17*$M1_pitch)]"
set M1_stappling    ""
...
set M2_stappling   	"-stapling {0.202 M1}"
set M2_spacing    	"[expr $M2_min_space + 5*$M2_pitch ]"


#M3:
set M3_set2set_dist         $M1_set2set_dist
set M3_spacing              "[expr 3*$M3_pitch-$M3_min_space] [expr 12*$M3_pitch-$M3_min_space] [expr 3*$M3_pitch-$M3_min_space]"
set M3_offset               [expr 12*$M3_pitch-$M3_width/2]
set M3_stapling             ""

#M4:
set M4_set2set_dist         [expr 19*$M4_pitch]
set M4_offset               [expr $M2_offset]
set M4_spacing              "[expr $M4_min_space + 2*$M4_pitch ] [expr $M4_min_space + 1*$M4_pitch ] [expr $M4_min_space + 2*$M4_pitch ] "
set M4_stapling             "-stapling {0.240 M3}"
...
set M5_spacing              "[expr $M5_min_space + 7*$M5_pitch]"
...
#M6:
set M6_set2set_dist         [expr 0.76 + 2*$M6_pitch]
set M6_offset               $M4_offset
set M6_spacing              "[expr $M6_min_space + 2*$M6_pitch] [expr $M6_min_space + 2*$M6_pitch] [expr $M6_min_space + 2*$M6_pitch]"
...
set M8_spacing              "[expr $M8_min_space + 2*$M8_pitch] [expr $M8_min_space + 2*$M8_pitch] [expr $M8_min_space + 2*$M8_pitch]"
...
set M10_spacing              "[expr $M10_min_space + 2*$M10_pitch] [expr $M10_min_space + 2*$M10_pitch] [expr $M10_min_space + 2*$M10_pitch]"
...
foreach inst_dpo [get_db insts -if {.base_cell.base_class == block}] {
  create_route_blockage -rects [join [get_computed_shapes [get_db $inst_dpo .bbox] SIZE 0.48]] -layers {M1 M2 M3 M4 M5 M6 M7 M8 M9 VIA0 VIA1 VIA2 VIA3 VIA4 VIA5 VIA6 VIA7 VIA8}  -name temp_mem_blockages
}
