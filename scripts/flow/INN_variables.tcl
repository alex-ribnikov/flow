...
if { [regexp "snpsn5|tsmcn5|inext|brcm5|nextcore|nxt080|nxt013" $proj] } {
...
# Flow
###############################################
if {[regexp "brcm3" $proj]} {
    set_db design_flow_effort standard ;					# (default : standard)
    set_db design_cong_effort medium
} else {
...
set_db opt_via_pillar_effort low ;					# (default : low)

set_db design_pessimistic_mode true	;			# (default : false)

# Prevent assign buffers deletion
set_db init_no_new_assigns true        ;     # (default : false)
...
if {[info exists DEFAULT_ROW_PATTERN] && $DEFAULT_ROW_PATTERN != ""} {set_db floorplan_default_row_pattern_site $DEFAULT_ROW_PATTERN}


if {[regexp "brcm3" $proj]} {
set_db floorplan_first_row_site_index 0
set_db floorplan_last_row_site_index 0

    set_db floorplan_row_site_height computed ;					# (default : any)
    set_db floorplan_row_height_multiple 2
    set_db floorplan_row_height_increment_corner_to_corner 3
    set_db floorplan_row_height_increment_in_corner_to_corner 0 
    set_db floorplan_row_height_increment_in_corner_to_in_corner 5
...
if { [regexp "snpsn5|tsmcn5|inext|brcm5|brcm3|nextcore|nxt080|nxt013" $proj] } {
...
# Pin constraints
# TODO: These constraints should only be implemented on NDR (A) ports. 
#       Need to add some conditioning
#       This may not be the best place to set it
###############################################
if {[regexp {21.1} [get_db program_version]] && 0} {
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M17 -width 0.45 -depth 1.8"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate AP -width 1.8 -depth 1.8"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M1 -width 0.02 -depth 0.13"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M2 -width 0.02 -depth 0.144"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M3 -width 0.02 -depth 0.208"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M4 -width 0.02 -depth 0.208"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M5 -width 0.038 -depth 0.395"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M6 -width 0.08 -depth 0.188"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M7 -width 0.076 -depth 0.198"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M8 -width 0.08 -depth 0.188"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M9 -width 0.076 -depth 0.198"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M10 -width 0.08 -depth 0.188"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M11 -width 0.076 -depth 0.198"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M12 -width 0.062 -depth 0.354"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M13 -width 0.062 -depth 0.354"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M14 -width 0.126 -depth 0.453"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M15 -width 0.126 -depth 0.453"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -pinTemplate M16 -width 0.45 -depth 1.8"
    eval_legacy "setPinConstraint -cell [get_db designs .name] -side all -corner_to_pin_distance 10 -spacing 2"
}
...
if {[info exists SCAN] && $SCAN == "false"} {
	...
	if { [regexp "brcm|nextcore|nxt080|nxt013" $proj] } {
...
set_db reorder_scan_allow_swapping true ; # (default : false)
set_db place_global_reorder_scan true ; 				# (default : true)
set_db place_global_clock_gate_aware true ;				# (default : true)
#set_db place_global_clock_power_driven_effort standard ;		# (default : low)
#set_db place_global_activity_power_driven true ;			# (default : false)
#set_db place_hard_fence true ;						# (default : true)
set_db place_detail_swap_eeq_cells true			;               # (default : false)
...
if {[regexp {21.1} [get_db program_version]]} {
	#in version 20.1 this is none
#	set_db place_global_auto_blockage_in_channel  none ;            # (default : partial)
}
...
#    -min_jog_height 3 \
#}
#IB: Fix pin access issues:
eval_legacy {setPlaceMode -expAdvPinAccess true}
eval_legacy {setPlaceMode -viaInPin true}
...
if {[regexp "brcm3" $proj]} {
    ...
    set_db add_endcaps_wall_pitch 95.76
    set_db add_endcaps_wall_to_convex_corner_spacing 0.768
...
# add halo to FF 9 site width
foreach base_cell [get_db [get_db base_cells *FF* ] .name] {set_cell_padding -cells $base_cell  -left_side 9 }


# cell_edge_type
###############################################
if {[file exists scripts/flow/cell_edge_type.${PROJECT}.tcl]} {
	puts "-I- sourcing cell_edge_type.${PROJECT}.tcl"
	source -e -v scripts/flow/cell_edge_type.${PROJECT}.tcl
}
...
set_db opt_multi_bit_flop_opt true ;					# (default : splitOnly)
...
set_db opt_time_design_compress_reports true ;				# (default : true)
...
set_db opt_preserve_user_route_type_constraints true ;			# (default : true)
...
if {[info exists USEFUL_SKEW] && $USEFUL_SKEW == "true"} {
   if {[regexp {20.1} [get_db program_version]]} {
	...
	set_db opt_skew true
	set_db opt_skew_no_boundary true ;				# (default : false)
	if { [regexp "nextcore" $PROJECT] } {
	    set_db opt_skew_max_allowed_delay 0.50 ;
	} else {
	    set_db opt_skew_max_allowed_delay 0.150 ;
	}
   }
} else {
   if {[regexp {20.1} [get_db program_version]]} {
...
if {![info exists STAGE ] || $STAGE != "floorplan"} {
    ...
    set_db cts_max_fanout 32 ;					       # (default : 100)
    set_db cts_top_fanout_threshold 124 ;  			       # (default : unset)
    ...
    set_db cts_route_type_auto_trim false  ;			       # (default : true)
    set_db cts_max_source_to_sink_net_length_leaf 30 ;                 # (default : default)

    
#    set_db cts_fix_clock_sinks soft ;			       # (default : false)
#    set_db cts_post_route_repair_skew_by_buffering true ;     # (default : false)
#    set_db cts_post_route_enable_routing_eco true ;           # (default : false)
    ...
    ## Layer Adherence
    # set_db cts_routing_preferred_layer_effort high
    if {[regexp {20.1} [get_db program_version]]} {
        ...
        set_db cts_write_routing_correlation_report_to_log true ;	       # (default : false)
        set_db cts_enable_routing_correlation_report true ;	               # (default : false)
    }
}
...
# 12/03/2024 Royl : need to move this to the setup file
if {[get_db route_types .name clockRouteTop] == ""} {
  if { [regexp "brcm5|nextcore|nxt080|nxt013|inext" $proj]  } {
  ...
  } elseif { [regexp "brcm3" $proj]  } {
     create_route_type -name clockRouteTop -top_preferred_layer $MAX_ROUTING_LAYER -bottom_preferred_layer [expr $MAX_ROUTING_LAYER -1] -preferred_routing_layer_effort high -route_rule NDR_A__m78910111213_p076_SHLD -shield_net VSS
  } else {
...
if {[get_db route_types .name clockRouteTrunk] == ""} {
  if { [regexp "brcm5|nextcore|nxt080|nxt013|inext" $proj]  } {
      if {$MAX_ROUTING_LAYER < 13} {
           create_route_type -name clockRouteTrunk -top_preferred_layer  $MAX_ROUTING_LAYER -bottom_preferred_layer [expr $MAX_ROUTING_LAYER -1] -preferred_routing_layer_effort high -route_rule NDR_A__m6_p080__m7_p076__m8_p080__m9_p076__m10_p080__m11_p076_SHLD -shield_net VSS
      } else {
#     create_route_type -name clockRouteTrunk -top_preferred_layer $MAX_ROUTING_LAYER -bottom_preferred_layer 10 -preferred_routing_layer_effort high -route_rule NDR_A__m6_p080__m7_p076__m8_p080__m9_p076__m10_p080__m11_p076_SHLD -shield_net VSS
           create_route_type -name clockRouteTrunk -top_preferred_layer [expr $MAX_ROUTING_LAYER -2] -bottom_preferred_layer [expr $MAX_ROUTING_LAYER -3] -preferred_routing_layer_effort high -route_rule NDR_A__m6_p080__m7_p076__m8_p080__m9_p076__m10_p080__m11_p076_SHLD -shield_net VSS
      }
  } elseif { [regexp "brcm3" $proj]  } {
      if {$MAX_ROUTING_LAYER < 13} {
           create_route_type -name clockRouteTrunk -top_preferred_layer  $MAX_ROUTING_LAYER -bottom_preferred_layer [expr $MAX_ROUTING_LAYER -1] -preferred_routing_layer_effort high -route_rule NDR_A__m78910111213_p076_SHLD -shield_net VSS
      } else {
#     create_route_type -name clockRouteTrunk -top_preferred_layer $MAX_ROUTING_LAYER -bottom_preferred_layer 10 -preferred_routing_layer_effort high -route_rule NDR_A__m6_p080__m7_p076__m8_p080__m9_p076__m10_p080__m11_p076_SHLD -shield_net VSS
           create_route_type -name clockRouteTrunk -top_preferred_layer [expr $MAX_ROUTING_LAYER -2] -bottom_preferred_layer [expr $MAX_ROUTING_LAYER -3] -preferred_routing_layer_effort high -route_rule NDR_A__m78910111213_p076_SHLD -shield_net VSS
      }
  } else {
     create_route_type -name clockRouteTrunk -top_preferred_layer $MAX_ROUTING_LAYER -bottom_preferred_layer 10 -preferred_routing_layer_effort high -route_rule 2w_2s -shield_net VSS
  }
}

if {[get_db route_types .name clockRouteLeaf] == ""} {
   if { [regexp "brcm5|nextcore|nxt080|nxt013|inext" $proj]  } {
      if {$MAX_ROUTING_LAYER < 12} {
          create_route_type -name clockRouteLeaf -top_preferred_layer $MAX_ROUTING_LAYER -bottom_preferred_layer 8 -preferred_routing_layer_effort high -route_rule NDR_A__m6_p080__m7_p076__m8_p080__m9_p076__m10_p080__m11_p076
      } else {
#     create_route_type -name clockRouteLeaf -top_preferred_layer [expr min($MAX_ROUTING_LAYER,10)] -bottom_preferred_layer 8 -preferred_routing_layer_effort high -route_rule NDR_A__m6_p080__m7_p076__m8_p080__m9_p076__m10_p080__m11_p076
          create_route_type -name clockRouteLeaf -top_preferred_layer 11 -bottom_preferred_layer 8 -preferred_routing_layer_effort high -route_rule NDR_A__m6_p080__m7_p076__m8_p080__m9_p076__m10_p080__m11_p076
      }
   } elseif { [regexp "brcm3" $proj] } {
      if {$MAX_ROUTING_LAYER < 12} {
          create_route_type -name clockRouteLeaf -top_preferred_layer $MAX_ROUTING_LAYER -bottom_preferred_layer 8 -preferred_routing_layer_effort high -route_rule NDR_A__m78910111213_p076
      } else {
#     create_route_type -name clockRouteLeaf -top_preferred_layer [expr min($MAX_ROUTING_LAYER,10)] -bottom_preferred_layer 8 -preferred_routing_layer_effort high -route_rule NDR_A__m6_p080__m7_p076__m8_p080__m9_p076__m10_p080__m11_p076
          create_route_type -name clockRouteLeaf -top_preferred_layer 11 -bottom_preferred_layer 8 -preferred_routing_layer_effort high -route_rule NDR_A__m78910111213_p076
      }
   } else {
     create_route_type -name clockRouteLeaf -top_preferred_layer [expr min($MAX_ROUTING_LAYER,10)] -bottom_preferred_layer 8 -preferred_routing_layer_effort high
   }
}
if {![info exists STAGE ] || $STAGE != "floorplan"} {
...
if {[regexp {20.1} [get_db program_version]]} {
	...
	set_db opt_hold_ignore_path_groups {in2reg reg2out in2out} ;	# (default : "")
}
#set_db opt_signoff_hold_target_slack -0.010 ;				# (default : -1000.0)
...
if {[regexp {21.1} [get_db program_version]]} {
#in version 20.1 this is false
set_db timing_apply_exceptions_to_data_check_related_pin false 
}
...
if { [info exists DEFAULT_CCOPT_VIEW] && [llength [get_db analysis_views $DEFAULT_CCOPT_VIEW]] > 0 } {
    puts "-I- Set cts_primary_delay_corner: $DEFAULT_CCOPT_VIEW"
    set_db cts_primary_delay_corner [get_db [get_db analysis_views $DEFAULT_CCOPT_VIEW] .delay_corner.name]    
}
...
if {![info exists STAGE] || ($STAGE != "chip_finish" && $STAGE != "floorplan" )} {
  ...
  foreach _av [lsort -uniq [concat $scenarios(setup) $scenarios(hold) $scenarios(dynamic) $scenarios(leakage)]] {
	...
	regsub {_\d+} [get_db [get_db analysis_views $_av] .delay_corner.late_rc_corner.name] "" _rc
	puts "INFO: set_socv_rc_variation_factor $rc_corner($_rc,rc_variation) -view $_av -late -early"
	set_socv_rc_variation_factor $rc_corner($_rc,rc_variation) -view $_av -late -early
  }
}
set_db timing_enable_spatial_derate_mode true ;		        # (default : false)
...
#set parasitic_explorer_enable_analysis true ; Enable detailed analysis of Rs and Cs in the design
...
#   set_db extract_rc_effort_level high ;					# (default : medium)    
   set_db extract_rc_effort_level medium ;					# !!! To save runtime
}
...
set_db power_disable_static false ;					# (default : false)
...
#set_default_switching_activity -macro_activity 0.3
...
set_db add_fillers_scheme cellFirst 
set_db add_fillers_keep_fixed true
if {[info exists ADD_FILLERS_SWAP_CELL]} {set_db add_fillers_swap_cell $ADD_FILLERS_SWAP_CELL}
set_db add_fillers_eco_mode true 			;		# (default : false)

#eval_legacy "setPlaceMode -place_detail_check_implant_across_rows_strict six"
if {[regexp "brcm3" $proj]} {
} else {
    eval_legacy "setFillerMode -minHole true"
}
...
#set_db add_fillers_avoid_abutment_patterns "1:1 1:2 2:1"
...
# Lec
###############################################
set_db write_lec_directory_naming_style  out/fv/$STAGE
...
if {[info exists VIA_PILLARS] && $VIA_PILLARS == "true" } {
	...
	source scripts/flow/via_pillars.${PROJECT}.tcl
}
...
if {![info exists INNOVUS_FROM_GENUS]  || $INNOVUS_FROM_GENUS == "false"} {
    if {[info exists OCV] && $OCV == "socv" && $STAGE != "floorplan" && $STAGE != "chip_finish" } {
	puts "-I- setting OCV socv"
        set_db timing_analysis_socv true ;					# (default : false)
	if {![regexp {22} [get_db program_version]]} {
	    # error when running set_timing_derate   -net -incr
	    # innovus return error when -net and -incr are together .
            set_db timing_derate_incremental_adjust_additive_backward_compatibility false
	}
	source -v -e ./scripts/flow/derating.${PROJECT}.tcl
	
    } elseif {[info exists OCV] && $OCV == "flat" && $STAGE != "floorplan" && $STAGE != "chip_finish" } {
   ...
   create_clock_tree_spec -out_file scripts_local/${DESIGN_NAME}_create_clock_tree_spec.tcl
   source scripts_local/${DESIGN_NAME}_create_clock_tree_spec.tcl
